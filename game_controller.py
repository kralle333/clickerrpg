...
class GameController:
    ...
    def __init__(self) -> None:
        ...
        forest = Area("Forest", (0, 118, 2), [0, 1, 1, 2, 1, 3, 2, 1, 3, 2])
        forest.add_enemy(
        ...
        forest.add_enemy(Enemy("Friendly Tree",
                         "Forest Tree.png", 200, 320, 10, 30, 5, 20, 20))
        forest.level_min = 1
        forest.level_max = 3
        forest.set_boss(Enemy(
            "Forest Boss", "Boss Continental Turtle Rukkha.png", 1000, 160, 50, 100, 0, 500, 100))
        self.areas.append(forest)

        desert = Area("Desert", (210, 187, 0), [0, 1, 2, 1, 2, 3, 2, 1, 3, 2])
        desert.add_enemy(Enemy("Snek", "Desert Sand Snake",
                         100, 60, 5, 10, 50, 4, 10))
        desert.add_enemy(
            Enemy("Tentacles", "Desert Rock Bunny.png", 100, 15, 1, 10, 15, 50, 100))
        desert.add_enemy(
        ...
        ice = Area("Frozen Cave", (	0, 97, 159), [
                   0, 1, 2, 1, 2, 3, 2, 1, 3, 2])
        ice.add_enemy(Enemy("Ice Bat", "Ice Cave Bat",
                      100, 60, 5, 10, 50, 4, 10))
        ice.add_enemy(
            Enemy("Ice Lion", "Ice Lion.png", 100, 15, 1, 10, 15, 50, 100))
        ice.add_enemy(
            Enemy("Ice Golem", "Ice Golem.png", 300, 120, 10, 100, 30, 20, 15))
        ice.add_enemy(
            Enemy("Frozen Birdie", "Ice Avian.png", 500, 30, 1, 20, 15, 50, 100))
        ice.level_min = 15
        ice.level_max = 30
        ice.set_boss(Enemy(
        ...
        self.player = Player(50, self.game_font)
    ...
    def set_current_enemy(self, enemy):
        ...
        self.current_enemy.load_gfx(self.game_font)
    ...
    def update(self, screen):
        ...
        # game over screen
        if self.ticks < 0:
            pygame.draw.rect(screen, (255, 40, 40), (0, 0, 480, 480))
            self.game_font.render_to(screen, (160, 220),
                                     "YOU'RE DEAD", (255, 255, 255))
            self.ticks += 1
            return

        pygame.draw.rect(screen, self.current_area.bg_color,
                         (0, 0, 480, 480))
        ...
        if self.current_enemy.current_hp <= 0:
            if self.player.on_defeat(self.current_enemy):
                self.floating_texts.append(FloatingText("LEVEL UP!",self.game_font,"level",180,200))
